...
public class LogicServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (currentSession.getAttribute("field") == null) {
            resp.sendRedirect("/index.jsp"); // или другой стартовый URL
            return;
        }

        // БЛОКИРОВКА: если игра уже закончена, просто перенаправляем на главную
        if (currentSession.getAttribute("winner") != null || currentSession.getAttribute("draw") != null) {
    ...
    private boolean checkWin(HttpServletResponse response, HttpSession currentSession, Field field) throws IOException {
        ...
        if (Sign.CROSS == winner || Sign.NOUGHT == winner) {
            ...
            List<Integer> winnerOffsets = field.getWinnerOffsets();
            currentSession.setAttribute("winnerOffsets", winnerOffsets);
    ...
    private Field extractField(HttpSession currentSession) {
        ...
        if (fieldAttribute == null) {
            ...
            throw new RuntimeException("Session is expired or broken, please restart the game");
        }
